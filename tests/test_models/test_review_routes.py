...
class TestClient(unittest.TestCase):
    ...
    def test_get_places_reviews(self):
        """ Test places endpoint"""
        ...
        storage.save()
        rv = self.client.get(f'/api/v1/places/{new_place.id}/reviews')
        data = json.loads(rv.data)
        assert len(data) > 0
        assert data[0].get('__class__') == "Review"

    def test_get_a_places_reviews(self):
        ...
        storage.new(new_review)
        storage.save()
        rv = self.client.get(f'/api/v1/reviews/{new_review.id}')
        data = json.loads(rv.data)
        assert len(data) > 0
        assert data.get('__class__') == "Review"

    def test_delete_a_places_reviews(self):
        """ Test places endpoint"""
        new_state = State(name="Accra")
        storage.new(new_state)
        storage.save()
        ...
        storage.new(new_review)
        storage.save()
        rv = self.client.delete(f'/api/v1/reviews/{new_review.id}')
        assert b'{}' in rv.data

        rv = self.client.delete(f'/api/v1/reviews/{new_user.id}')
        data = json.loads(rv.data)
    ...
    def test_create_a_places_reviews(self):
        ...
        body = {
                "user_id": new_user.id,
                "text": "what a place to be"
        }

        rv = self.client.post(
                f'/api/v1/places/{new_place.id}/reviews', json=body
            )
